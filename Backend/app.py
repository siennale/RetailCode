...
def set_product(barcode, name, price, tax, total_qty, total_sold_qty, total_instock_qty, conn):
	c = conn.cursor()
	c.execute("INSERT OR REPLACE INTO Products (product_barcode, product_name, product_selling_price, product_tax, product_sold_quantity,product_total_quantity,product_instock_quantity) VALUES (?,?,?,?,?,?,?)",(barcode, name, price, tax, total_qty, total_sold_qty, total_instock_qty))
	conn.commit()
	return c.lastrowid
...
def set_transaction(user_id, total_sale, total_item, conn):
	c = conn.cursor()
	transaction = (user_id, datetime.datetime.utcnow(), total_item, total_sale, 0 )
	c.execute("INSERT INTO Transactions (user_id, date, total_items, total_sale, bills_offered) VALUES(?,?,?,?,?)", transaction)
	conn.commit()
	return c.lastrowid

def set_sold_items(product_barcode, product_selling_price, quantity, transaction_id):
	c = conn.cursor()
	sold_item = (product_barcode, quantity)
	c.execute("INSERT INTO SoldItems (product_barcode, quantity, transaction_id) VALUES(?,?,?)", sold_item)
	conn.commit()
	return c.lastrowid
...
def POST_save_product():
	...
	try:
		with create_connection('retail.db') as conn:
			result = set_product(data["product_barcode"], data["product_name"], data["product_selling_price"], data["product_tax"], data["product_total_qty"],data["product_sold_qty"], data["product_instock_qty"],conn)
			return (str(result))
	except KeyError:
...
def POST_save_order():
	...
	try:
		with create_connection('retail.db') as conn:
			transaction_id = set_transaction(data.user_id, data.total_sale, data.total_items, conn)
			for barcode,qty in data:
				print (key + " " + value, flush=True)
				set_sold_items(barcode, qty, transaction_id, conn)
	except Error as ex:
		return jsonify({"result": ex})
